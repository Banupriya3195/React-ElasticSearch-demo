...
import "./Elsearch.scss"
import {useState,useEffect, useCallback} from 'react'
import AxiosApi from "../Api/AxiosApi";
import MenuItem from '@mui/material/MenuItem';
import { DataGrid } from "@mui/x-data-grid/DataGrid";
import { SourceColumn } from "../Components/TableSource";
...
const Elsearch=()=>{
    ...
    const [ApiData,setApiData]=useState<srcInterface[]>([]);
    const [ApiSearchData,setSearchApiData]=useState<srcInterface[]>([]);
    const [SearchVal, setSearchVal] = useState('legacy');
    ...
    const getAllTemplate=async()=>{
        try{
          const response:any= await AxiosApi.get(`source/getsourcedata`)
    ...
    const ApiHandlerSrc=useCallback(async(e:any)=>{
        ...
        try{
            const response:any= await AxiosApi.get(`source/search?keyword=${letter}`
            )
            const data=await response?.data;
